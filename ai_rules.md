...
## 14) Git Operations & Branch Safety (قواعد التعامل مع المستودع وحماية الفروع)

1. **الالتزام المحلي فقط (Local Commits Only)**:
   - يجب على الذكاء الاصطناعي تجهيز وإضافة جميع التعديلات محلياً (`git add` و `git commit`) دون محاولة الدفع التفاعلي (Interactive Push) الذي يتطلب إدخال بيانات الاعتماد في البيئة الخلفية غير التفاعلية للعميل الذكي.
2. **إرشادات الدفع للمستخدم (Push Instructions)**:
   - يجب على الذكاء الاصطناعي صياغة أمر الدفع الدقيق (مثل `git push origin main --force`) وتوجيه المستخدم لتشغيله بنفسه في محطته الطرفية (Terminal) حيث تتوفر بيانات اعتماده لـ GitHub.
3. **حماية الفروع (Branch Protection)**:
   - بعد أي عملية استعادة لفرع ملوث أو حل تعارض في شجرة الـ Git، يجب على الذكاء الاصطناعي توجيه المستخدم لحماية فرع `main` على GitHub بإضافة قواعد الحماية (مثل منع الدفع المباشر وقصر الدمج على PRs المجتازة للاختبارات).
