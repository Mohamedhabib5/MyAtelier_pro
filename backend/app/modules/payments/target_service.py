...
def search_payment_targets(db: Session, session: dict, query: str, branch_id: str | None = None) -> list[dict]:
    ...
    for booking in bookings:
        if not token or token in booking.booking_number.lower() or token in booking.customer.full_name.lower():
            results.append(
                {
                    'kind': 'booking',
                    'id': booking.id,
...
def get_customer_payment_target(db: Session, session: dict, customer_id: str, branch_id: str | None = None, ignore_payment_document_id: str | None = None) -> dict:
    ...
    if not bookings:
        ...
        if customer is None or customer.company_id != company.id:
            raise NotFoundError('ظ„ظ… ظٹطھظ… ط§ظ„ط¹ط«ظˆط± ط¹ظ„ظ‰ ط§ظ„ط¹ظ…ظٹظ„')
            ...
            lines.append(
                {
                    ...
                    'paid_total': float(line_paid_total(line, ignore_payment_document_id=ignore_payment_document_id)),
