...
def auto_post_payment_document(db: Session, actor: User, payment_document: PaymentDocument) -> JournalEntry:
    ...
    entry.lines = _build_payment_lines(total_amount, advances_amount, receivables_amount, cash_account.id, advances_account.id, receivables_account.id, payment_document.payment_number)
    repo.add_journal_entry(entry)
    db.flush()
    record_audit(db, actor_user_id=actor.id, action='accounting.payment_document_auto_posted', target_type='journal_entry', target_id=entry.id, summary=f'Auto-posted payment document {payment_document.payment_number} to journal {entry.entry_number}', diff={'payment_document_id': payment_document.id, 'total_amount': float(total_amount), 'advances_amount': float(advances_amount), 'receivables_amount': float(receivables_amount)})
    return entry


def reverse_linked_payment_document_entry(db: Session, actor: User, payment_document: PaymentDocument, reverse_date: date) -> JournalEntry | None:
    ...
    reversal.lines = [
        JournalEntryLine(line_number=index, account_id=line.account_id, description=line.description, debit_amount=line.credit_amount, credit_amount=line.debit_amount)
    ...
    db.flush()
    record_audit(db, actor_user_id=actor.id, action='accounting.payment_document_entry_reversed', target_type='journal_entry', target_id=entry.id, summary=f'Reversed linked journal entry {entry.entry_number} for payment document {payment_document.payment_number}', diff={'payment_document_id': payment_document.id, 'reversal_entry_number': reversal.entry_number})
    return reversal
...
def _build_payment_lines(total_amount: Decimal, advances_amount: Decimal, receivables_amount: Decimal, cash_account_id: str, advances_account_id: str, receivables_account_id: str, payment_number: str) -> list[JournalEntryLine]:
    description = f'Payment document {payment_number}'
    lines = [JournalEntryLine(line_number=1, account_id=cash_account_id, description=description, debit_amount=total_amount, credit_amount=ZERO)]
    line_number = 2
    if advances_amount > ZERO:
        lines.append(JournalEntryLine(line_number=line_number, account_id=advances_account_id, description=description, debit_amount=ZERO, credit_amount=advances_amount))
        line_number += 1
    if receivables_amount > ZERO:
        lines.append(JournalEntryLine(line_number=line_number, account_id=receivables_account_id, description=description, debit_amount=ZERO, credit_amount=receivables_amount))
    return lines
