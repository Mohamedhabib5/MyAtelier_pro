import { Alert, Box, Chip, FormControlLabel, Grid, Stack, Switch, Tab, Tabs, TextField, Typography } from '@mui/material';
  ...
  return (
    <Stack spacing={3}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider', pb: 1 }}>
        ...
        <Tabs value={activeTab} onChange={(_, val) => setActiveTab(val)} sx={{ mt: 2 }}>
          <Tab label={language === 'ar' ? 'المحاسبة' : 'Accounting'} id='accounting-subtab-main' />
          <Tab label={language === 'ar' ? 'تسوية النقدية' : 'Cash Reconciliation'} id='accounting-subtab-reconciliation' />
        </Tabs>
      </Box>
